...
#ifdef SUPPORT_AUDIO_RAW
			if (raw_info != NULL && raw_info->raw_enable == 1 && card == CARD_A1X_HDMI)
			{
                                // only hdmi raw mode need this para
                                ALOGV("SUPPORT_AUDIO_RAW, raw_enable=%d, SampleRate=%d, mode=%d", raw_info->raw_enable, raw_info->sample_rate, raw_info->raw_mode);
...
#ifdef SUPPORT_AUDIO_RAW
                && raw_info != NULL && raw_info->raw_enable == 0
#endif
				)
            {
...
//	if ((adev->mode == AUDIO_MODE_IN_CALL&&adev->out_device==AUDIO_DEVICE_OUT_BLUETOOTH_SCO_HEADSET)||adev->mode == AUDIO_MODE_MODE_FACTORY_TEST || adev->mode == AUDIO_MODE_FM)
    ...
    */
    if ((raw_info.raw_enable != adev->raw_enable) ||
            ((raw_info.raw_enable == 1) && 
                (raw_info.sample_rate != adev->raw_samplerate || raw_info.raw_mode != adev->raw_mode)))
    {
        ALOGD("### Raw mode changing! Need to out_standy");
        ALOGD("### Old Raw Settings(adev), raw_enable=%d, raw_mode=%d,sampleRate=%d", adev->raw_enable, adev->raw_mode, adev->raw_samplerate);
        ALOGD("### New Raw Settings(raw_info), raw_enable=%d, raw_mode=%d, sampleRate=%d", raw_info.raw_enable, raw_info.raw_mode, raw_info.sample_rate);
        out_standby(stream);
        adev->raw_enable = raw_info.raw_enable;
        adev->raw_mode = raw_info.raw_mode;
        adev->raw_samplerate = raw_info.sample_rate;
    }
#endif

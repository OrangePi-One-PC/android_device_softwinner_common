...
#define UART_PORT_PREFIX   "/dev/ttyS"
...
void userial_vendor_init(void)
{
    ...
    snprintf(vnd_userial.port_name, VND_PORT_NAME_MAXLEN, "%s%d", \
            UART_PORT_PREFIX, uart_id);

	snprintf(dev, sizeof(dev), "%s%d", UART_PORT_PREFIX, uart_id);
    if (property_set(BT_UART_PROPERTY, dev))
    {
        ALOGE("set property: %s failed...", BT_UART_PROPERTY);
    }
}
